...
float3 fresnelSchlickRoughness(float cosTheta, float3 startFresnelSchlick, float roughness){
    return startFresnelSchlick + (max(float3(1.0 - roughness, 1.0-roughness, 1.0-roughness), startFresnelSchlick) - startFresnelSchlick) * pow(1.0 - cosTheta, 5.0);
}

float3 processPointLight(PointLight light, float3 normals, float3 fragmentPosition, float3 worldViewVector, float3 startFresnelSchlick, float roughness, float metallic, float3 albedo){
    ...
    if(defaultFunction)
    {
        ...
        denominator = 4.0 * max(dot(normals, worldViewVector), 0.0) * max(dot(normals, processedLightPos), 0.0) + 0.0001;
...
float4 main(PixelShaderInput psInput) : SV_Target
{
    ...
    for (uint i = 0; i < 3; i++)
    {
        Lo += processPointLight(lightsInfos[i], normal, psInput.position, worldViewVector, startFresnelSchlick, roughness, metallic, psInput.color);
    }
    float3 fresnelRoughness = fresnelSchlickRoughness(max(dot(normal, worldViewVector), 0.0), startFresnelSchlick, roughness);
